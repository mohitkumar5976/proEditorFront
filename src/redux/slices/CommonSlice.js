...
export const commonSlice = createSlice({
  ...
  reducers: {
    ...
    addStickerItem: (state, action) => {
      state.stickerItems = action.payload;
    },
    handleCurrentIndex: (state, action) => {
      state.currentIndex = action.payload;
    },
    handleHistoryItems: (state, action) => {
      const newHistory = [
        ...state.historyItems.slice(0, state.currentIndex + 1),
        action.payload,
      ];
    ...
    deleteDroppedObject: (state, action) => {
      if (action.payload.type === "s") {
        ...
        state.textItems = state.textItems.filter(
      ...
      if (objectIndex !== -1) {
        switch (positionType) {
          ...
          case "TR":
            ...
            state.history = state.droppedItems;
            break;
    ...
    handleTouchDownOnObject: (state, action) => {
      state.isMoving = true;
      state.selectedObject = action.payload.object;
      const { eventX, eventY, object } = action.payload;
